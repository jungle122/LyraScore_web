...
| 模块三：练习计划与日志 | User | 创建有时限练习计划、N:M 关联乐谱、记录练习时长/BPM/心得、聚合统计（GROUP BY）、**AI 周报生成** |
...
5. **AI 周报**：`/logs/report` 把 stats JSON 喂给 qwen-turbo → 100-200 字教练点评 + 改进建议 + 鼓励收尾。AI 失败时本地拼接 fallback 文案
...
| AI 服务 | 阿里云百炼 qwen-turbo | OpenAI 兼容端点；点歌清洗 + 周报生成 |
...
2. **练习周报**：stats JSON → 100-200 字教练点评。temperature=0.7 让每次生成略有不同。

**关键设计：失败降级**。AI 接口可能因网络、限流、key 失效等原因失败。我们的设计是：
- 点歌：AI 失败 → 写入原文 + status=2 → 前端显示「AI 失败 / 待人工审核」+ 「重新清洗」按钮
- 周报：AI 失败 → 本地拼一段「累计 N 分钟，本周 N 分钟，加油」fallback 文案 → 业务永远不中断
...
**日志统计页**：3 张顶部统计卡 + **AI 练习周报卡片**（暖色渐变背景，含「生成本周总结」按钮和 AI 生成的文本）+ 「各曲累计时长」表（**GROUP BY 教学点**）+ 「近 7 日练习」表（**DATE() + GROUP BY 教学点**）+ 最近练习记录表（含删除按钮）。
...
3. **生成 AI 周报**：日志 → 滚到「🤖 AI 练习周报」卡片 → 「生成本周总结」 → 等待 1-3 秒
